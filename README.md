...
`build_data.py` pulls E. coli records within a bounding box covering the **American River, Lake Natoma, Folsom Lake, the Sacramento River (north to Knights Landing), and the Feather River (north to Yuba City)**, consolidates them by monitoring station (collapsing bank/replicate variants, dropping one-off study points, stormwater sumps, drains/bypasses, and discontinued sites), and keeps ongoing river/lake swim sites (last sampled since 2024). It uses the dataset's official **6-week geometric mean** where available. This yields the Lower American River stations plus Howe Ave, the Lake Natoma / Folsom Lake sites, and the new **Sacramento River** (below Verona, below Knights Landing, above Colusa Basin Drain) and **Feather River** (Feather @ Verona, Yuba River mouth) stations.
...
**The rivers are modeled as one connected network.** The **American** and **Feather** rivers flow into the **Sacramento** (at Sacramento and at Verona), with no dams between — so a float can **cross a confluence**: e.g. put in on the Feather near Nicolaus and take out on the Sacramento at Elkhorn. Each point carries a *reach mile* (along its own river) and a *network mile* (to the system mouth), and routes are drawn across reaches. **Only hydrologically-connected spots are offered:** a trip follows one continuous up/down flow line. Points on different branches (American vs Feather) or across a dam (the lakes — Nimbus/Folsom) can't be linked — you'd plan those as separate legs.
...
- Shows **distance** (river miles, or straight-line paddle miles; out-and-back is round-trip) and **float time from a discharge-based velocity**: mean velocity = live discharge ÷ estimated cross-sectional area (continuity, **V = Q ÷ A**), with the fastest travel down the **thalweg** (deep center) and slower water near the banks. The float-time range runs thalweg (fast) → mid-channel mean (typical), and the panel shows the estimated thalweg / mean / bank speeds and the basis (Q ÷ width × depth). Lakes and out-and-backs (where the current cancels) fall back to a **2–4 mph paddle** estimate. One-way trips add a live **shuttle drive time** — routed **parking-lot to parking-lot** (take-out → put-in), clickable to open **Google Maps driving directions**; out-and-back trips show **🚫 no shuttle**.
...
**Data / rebuild:** the source of truth is **`access_points_source.csv`** (columns: `id, name, segment, access_lat, access_lon, parking_lat, parking_lon, parking_fee, parking_info, walk_to_water, amenities, cautions, note`). `segment` is a **flowing reach** — `river` (American), `sacramento-river`, `feather-river` (float + shuttle, connected as a downstream network) — or a **lake** — `lake-natoma` / `folsom-lake` (flatwater above a dam — paddle, no shuttle). Edit it, then run `python3 build_access.py`, which fetches each river's centerline from **OpenStreetMap** (clipped to its corridor), orders it downstream→upstream, computes a per-reach mile and a network `flow_mi` (with the Feather/American confluences snapped onto the Sacramento), snaps each river `access_*` point on for its miles (lake points have none — straight-line paddle distances), and writes `docs/river_line.json` (multi-reach) + `docs/access_points.json`. New Sacramento/Feather access points (Discovery Park, Elkhorn, Verona, Knights Landing; Verona, Nicolaus, Shanghai Bend, Yuba City) have **approximate coordinates** to refine.
